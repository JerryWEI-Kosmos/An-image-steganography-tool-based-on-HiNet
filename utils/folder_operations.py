...
def select_and_copy_images_manually():
    folder1_path = 'Dataset/covers/'
    folder2_path = 'Dataset/secrets/'
    output_folder_path = 'Dataset/datas/'
    # 获取文件夹中的所有图片文件
...
# Web
def manage_folder_images(request_data):
    # 从请求数据中获取操作类型和相关信息
    ...
    if action == 'delete':
        ...
        try:
            shutil.copy(image_path, folder_path)
            return jsonify({'message': f"Image added successfully to {folder_path}."})
        except FileNotFoundError:
